<?php
     ...
     */
    class WC_Gateway_Midtrans_MIGS extends WC_Payment_Gateway {

      /**
       * Constructor
       */
      function __construct() {
        $this->id           = 'midtrans_migs';
        $this->icon         = apply_filters( 'woocommerce_midtrans_icon', '' );
        $this->method_title = __( 'Midtrans CC MIGS', 'woocommerce' );
        $this->has_fields   = true;
        $this->notify_url   = str_replace( 'https:', 'http:', add_query_arg( 'wc-api', 'WC_Gateway_Midtrans_MIGS', home_url( '/' ) ) );
      ...
      public function admin_options() { ?>
        <h3><?php _e( 'Midtrans CC MIGS', 'woocommerce' ); ?></h3>
        <p><?php _e('Allows online payments with MIGS channel using Midtrans.', 'woocommerce' ); ?></p>
      ...
      function init_form_fields() {
        ...
        $this->form_fields = array(
          'enabled' => array(
            ...
            'type' => 'checkbox',
            'label' => __( 'Enable Midtrans CC MIGS Payment', 'woocommerce' ),
            'default' => 'no'
          ),
          'title' => array(
